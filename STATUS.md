...
추가 수정: 어드민 문의 상세의 내부 메모 저장 시 `admin_name` 누락으로 422가 나던 문제를 API에서 세션/프로필 기반으로 보정하도록 해결했다.
...
| P1 | Google OAuth 운영 설정 | @backend/Scott | 🟡 부분 확인 | 기존 google/admin profile 확인. 현 배포 기준 실제 로그인 재수행 + last_login_at 갱신 확인 필요 |
...
- 47 errors → 0 errors
- warnings 19개는 잔존하지만 lint exit code는 0
- 주요 수정: scripts CommonJS lint override, React Compiler 지적 컴포넌트 구조/상태 업데이트 위치 정리, JSX quote escape, `prefer-const`
...
_최종 업데이트: 2026-08-11 | 업데이트 담당: @dev_cshdotcom_bot_

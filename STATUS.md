...
| 이슈 | 작업 | 브랜치 | 상태 | 블로킹 |
|------|------|--------|------|--------|
| #1 | Supabase 연결 확인 + /api/health | `be/supabase-setup` | 🔲 대기 | — |
| #2 | GET /api/speakers, /api/speakers/:id | `be/api-speakers` | 🔲 대기 | #1 완료 후 |
| #3 | GET /api/lectures, /api/lectures/:id | `be/api-lectures` | 🔲 대기 | #1 완료 후 |
| #4 | POST /api/inquiries + Resend | `be/api-inquiries` | 🔲 대기 | #1 완료 후 |
...
| 이슈 | 작업 | 브랜치 | 상태 | 블로킹 |
|------|------|--------|------|--------|
| #7 | 디자인 시스템 + 공통 컴포넌트 + 헤더/푸터 | `fe/design-system` | 🔲 대기 | — |
| #8 | 홈 랜딩 페이지 | `fe/home-page` | 🔲 대기 | #7 완료 후 |
| #9 | 강사 리스트/상세 | `fe/speakers-page` | 🔲 대기 | #7 + BE#2 완료 후 |
| #10 | 강연 커리큘럼 | `fe/lectures-page` | 🔲 대기 | #7 + BE#3 완료 후 |
| #11 | 문의하기 폼 | `fe/inquiry-page` | 🔲 대기 | #7 + BE#4 완료 후 |
| #12 | 고객지원 (FAQ/공지/회사소개) | `fe/support-pages` | 🔲 대기 | #7 완료 후 |
| #13 | Auth 페이지 (회원가입/로그인) | `fe/auth-pages` | 🔲 대기 | #7 + BE#5 완료 후 |
| #14 | 어드민 UI | `fe/admin-ui` | 🔲 대기 | #7 + BE#6 완료 후 |
...
| 이슈 | 작업 | 브랜치 | 상태 | 블로킹 |
|------|------|--------|------|--------|
| #15 | Playwright E2E 세팅 + GitHub Actions | `qa/e2e-setup` | 🔲 대기 | — |

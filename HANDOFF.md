...
## TAG-001: 전체 인사이트 태그 완비 (2025-03-19)
...
| 제목 | 추가된 태그 |
|------|-----------|
| AI가 내 일자리를 빼앗는다? | AI강연, IT, 미래, 리더십, 자기계발 |
| MZ세대와 함께 일하는 법 | MZ세대, 조직문화, 갈등관리, 리더십, 팀장 |
| 2026 기업이 가장 원하는 강연 TOP 5 | AI강연, ESG, 리더십, HR, 동기부여 |
| 강연 하나로 조직이 바뀔 수 있을까? | 조직문화, 리더십, HR, 경영전략 |
| 2025년 기업 교육, 무엇이 달라졌나 | HR, AI강연, 리더십, ESG, 업무스킬 |
| 행복하세요가 아니라 행복을 설계하세요 | 행복, 심리, 동기부여, 자기계발, 힐링 |
| 번아웃 직전의 팀 | 번아웃, 멘탈헬스, 심리, 조직문화, 회복탄력성 |
| MZ와 꼰대 사이 | MZ세대, 팀장, 리더십, 조직문화, 소통 |

### 코드 변경
- `report/page.tsx`: InsightCard 카드 하단에 태그 표시 + 강사 라우팅 연결 추가

### 태그 표시 현황
- `/insights/issue` 리스트: ✅ (featured 카드 + 아티클 리스트 모두)
- `/insights/report` 리스트: ✅ (카드 하단)
- `/insights/issue/[id]` 상세: ✅ (InsightDetail 컴포넌트에 이미 구현)
- `/insights/report/[id]` 상세: 미확인 → @backend에 확인 요청 필요

### 자동 생성 컨텐츠 태그 규칙 ⚠️
**스캇 요청:** "앞으로 자동으로 올라가는 컨텐츠도 다 하단에 키워드가 나와야 함"
→ @backend: Vercel Cron Job으로 자동 생성 시 meta.tags 반드시 포함 필요
→ 태그는 FIELD_ALIASES 키 or SPEAKER_FIELDS 값 기준 (강사 필터 라우팅 작동 조건)
→ 예시 태그: 리더십, HR, MZ세대, 번아웃, AI강연, ESG, 조직문화, 동기부여 등
